...
def CreateBoard():
    ...
    hiddenBoard= [['[ ]' for i in range(y)] for j in range(x)]
...
def CheckHit(x,y, cpuMove):
    if (board[y][x] == '[=]' and cpuMove):
        board[y][x] = '[#]'
        return True
    elif (hiddenBoard[y][x] == '[=]'):
        board[y][x] = '[X]'
    elif (board[y][x] == '[ ]'):
        if(cpuMove):
            board[y][x] = '[-]'
...
def CreateCpuBoats():
    ...
    placed = 0
    for item, boat in boatLengths.items():   
        boatX = random.randint(0,9-boat)
        boatY = random.randint(0,9)
        #plyInput = input("Enter the rotation boat coord: (in XY i.e. 43)")
        i = 0
        while i < boat: 
            hiddenBoard[boatY][boatX + i] = "[=]"
            i = i +1
        os.system('cls')
        PrintBoard()
        placed = placed+1  
